var validationObject = {
  ...
  twt_description: {
    selector: "SeoPage[twt_description]",
    minLength: 1,
    maxLength: 300
  },
}
...
$(document).ready(function() {
  $("[data-analyze-seo]").on('click', onAnalyse);
  onAnalyse();
});
...
function setError(key) {
  if (!$('[data-field-name="' + key + '"]').hasClass('error')) {
    $('[data-field-name="' + key + '"]').toggleClass('error').removeClass('complete').removeClass('incomplete');
  }
...
function setComplete(key) {
  if (!$('[data-field-name="' + key + '"]').hasClass('complete')) {
    $('[data-field-name="' + key + '"]').toggleClass('complete').removeClass('error').removeClass('incomplete');
  }
}

function setIncomplete(key) {
  if (!$('[data-field-name="' + key + '"]').hasClass('incomplete')) {
    $('[data-field-name="' + key + '"]').toggleClass('incomplete').removeClass('complete').removeClass('error');
  }
}
